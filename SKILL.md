...
这个 Skill 不是单个图片接口的堆叠，而是把“证件照制作”整理成一个可直接调用的工作流：
...
优先目标不是解释接口，而是**把用户自然语言稳定映射到正确的处理链路和参数**。
...
3. **自动查找对应尺寸**（见下方尺寸映射表）
4. 识别任务类型：证件照 / 抠图 / 换底 / 增强 / 排版 / 编辑
5. 直接在 Python 中执行 `call_api()` 函数
6. 获取 `image_url` 或相关结果字段返回给用户
7. 附上成功提示
...
本 Skill 内置演示凭据，安装后可直接体验。以下内容主要用于 Skill 运行和调试，不建议在对外宣传材料中直接展示：
